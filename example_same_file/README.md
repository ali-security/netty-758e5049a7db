...
[`../../sdk_agents/breaking/DEBUG.md`](../../../sdk_agents/breaking/DEBUG.md)):
...
([`models/breaking_questions.py`](../../../models/breaking_questions.py)):
...
The verdict (`analyse_answers`) acts **only** on `true` — a question that fires.
`false` and `null` (can't-determine) both mean "did not fire" and are
...
Each run writes a YAML results file (default `<dataset dir>/calibration_results.yaml`,
override with `--results`). It's **created or merge-updated** — counts
**accumulate across runs** (this run's `attempts`/`failures` are added onto the
stored totals), and your `note:` fields and comments are kept:
...
- **Keyed by `md5(used_prompt)`.** Rewording a question changes its md5, so the
  run writes under a *new* key and the old rows are **orphaned** — a mechanical
  enforcement of DEBUG.md's "a reword breaks trace matching, so re-collect." The
  literal `used_prompt` text is stored alongside so the file is self-describing.
...
Like `suite.py`, this runs with `source_path` pointing at a non-existent dir and
`vuln_info=None`, so the `code_impact_analyzer` sub-agent reports "source not
available" and code-impact questions (`API_SIGNATURE_CHANGE`,
