...
`info.answers` away, so it can't tell you _which_ question was miscalibrated.
...
link:../../../sdk_agents/breaking/DEBUG.md[`../../sdk_agents/breaking/DEBUG.md`]):

* *Bucket A* -- the agent answered a question _wrong_ -> reword it / feed the
  sub-agent better evidence.
* *Bucket B* -- every answer is right but the _verdict_ is wrong -> re-tier the
  question or re-sweep the threshold.

== File format

One file per question, named after its `BreakingQuestion` member
(link:../../../models/breaking_questions.py[`models/breaking_questions.py`]):
...
stats. A hard negative is a near-miss where the question plausibly _could_ fire
...
* gold `false` *or* `null` -> the agent may answer *either* `false` or `null`.
...
`UPSTREAM_CALLS_BREAKING`) return can't-determine when no source _positively_
...
`attempts`/`failures` are added onto the stored totals), and your `note:` fields
...
* *Keyed by `md5(used_prompt)`.* Rewording a question changes its md5, so the
  run writes under a _new_ key and the old rows are *orphaned* -- a mechanical
...
* *YAML, not JSON*, precisely because the file has two writers: the harness owns
  the counts, you own the notes. Block scalars (`note: |`) make multiline notes
  painless and `ruamel` round-trips them (and your comments) untouched.
